...
describe("Queue and Worker Functions", () => {
  ...
  describe("Worker Processing", () => {
    ...
    it("should handle multiple task types", async function () {
      ...
      expect(processedTasks.filter((t) => t.startsWith("a:")).length).to.equal(
        2,
      );
      expect(processedTasks.filter((t) => t.startsWith("b:")).length).to.equal(
        1,
      );
      expect(processedTasks.filter((t) => t.startsWith("c:")).length).to.equal(
        1,
      );
    });
  ...
  describe("Concurrency and Performance", () => {
    it("should process tasks concurrently", async function () {
      ...
      const taskExecutions = startTimes
        .map((start, i) => ({
          start,
          end: endTimes[i]!,
        }))
      ...
      for (let i = 0; i < taskExecutions.length; i++) {
        ...
        for (let j = 0; j < taskExecutions.length; j++) {
          ...
          if (i !== j && taskJ.start < taskI.end && taskJ.end > taskI.start) {
            concurrent++;
          }
        }
