...
describe("Run Data API", () => {
  ...
  describe("POST /api/v1/runs/:runId/data", () => {
    ...
    it("should create run data with tags", async () => {
      ...
      expect(response.data.tags).to.deep.equal([
        "order",
        "status",
        "important",
      ]);
    ...
    it("should return 404 for non-existent run", async () => {
      const response = await client.post(
        "/api/v1/runs/00000000-0000-0000-0000-000000000000/data",
        {
          taskId,
          key: "test-key",
          value: { test: "data" },
  ...
  describe("GET /api/v1/runs/:runId/data", () => {
    ...
    it("should get all run data", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?includeAll=true`,
      );
    ...
    it("should filter by key", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?key=user-data&includeAll=true`,
      );
    ...
    it("should filter by tags", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?tags=config&includeAll=true`,
      );
    ...
    it("should filter by key prefix", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?keyStartsWith=user&includeAll=true`,
      );
    ...
    it("should support pagination", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?limit=2&offset=1&includeAll=true`,
      );
    ...
    it("should sort by created date", async () => {
      const response = await client.get(
        `/api/v1/runs/${runId}/data?sortBy=created_at&sortOrder=desc&includeAll=true`,
      );
  ...
  describe("PATCH /api/v1/runs/:runId/data/:dataId/tags", () => {
    ...
    it("should update tags", async () => {
      const response = await client.patch(
        `/api/v1/runs/${runId}/data/${dataId}/tags`,
        {
          remove: ["original", "tag"],
    ...
    it("should return 404 for non-existent data", async () => {
      const response = await client.patch(
        `/api/v1/runs/${runId}/data/non-existent-id/tags`,
        {
          add: ["test"],
        },
      );
  ...
  describe("DELETE /api/v1/runs/:runId/data", () => {
    ...
    it("should delete data by key", async () => {
      const response = await client.delete(
        `/api/v1/runs/${runId}/data?key=temp-data`,
      );
    ...
    it("should delete data by id", async () => {
      ...
      const listResponse = await client.get(
        `/api/v1/runs/${runId}/data?key=temp-data&limit=1&includeAll=true`,
      );
      const dataId = listResponse.data.data[0].id;

      const response = await client.delete(
        `/api/v1/runs/${runId}/data?id=${dataId}`,
      );
      ...
      const newListResponse = await client.get(
        `/api/v1/runs/${runId}/data?includeAll=true`,
      );
