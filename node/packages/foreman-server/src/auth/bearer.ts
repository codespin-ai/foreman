...
export function getBearerAuthConfig(): BearerAuthConfig {
  const token = process.env.FOREMAN_API_KEY;
  const enabled = process.env.FOREMAN_API_KEY_ENABLED === "true" || !!token;
...
export function validateBearerToken(
  ...
  if (!config.token) {
    logger.error(
      "Bearer authentication is enabled but FOREMAN_API_KEY is not set",
    );
    return {
      success: false,
      error: new Error(
