...
export function mapRunToDb(run: Partial<Run>): Partial<RunDbRow> {
  ...
  if (run.inputData !== undefined)
    dbRow.input_data = run.inputData as Record<string, unknown>;
  if (run.outputData !== undefined)
    dbRow.output_data = run.outputData as Record<string, unknown>;
  if (run.errorData !== undefined)
    dbRow.error_data = run.errorData as Record<string, unknown>;
  if (run.metadata !== undefined)
    dbRow.metadata = run.metadata as Record<string, unknown>;
  if (run.totalTasks !== undefined) dbRow.total_tasks = run.totalTasks;
  if (run.completedTasks !== undefined)
    dbRow.completed_tasks = run.completedTasks;
...
export function mapTaskToDb(task: Partial<Task>): Partial<TaskDbRow> {
  ...
  if (task.inputData !== undefined)
    dbRow.input_data = task.inputData as Record<string, unknown>;
  if (task.outputData !== undefined)
    dbRow.output_data = task.outputData as Record<string, unknown>;
  if (task.errorData !== undefined)
    dbRow.error_data = task.errorData as Record<string, unknown>;
  if (task.metadata !== undefined)
    dbRow.metadata = task.metadata as Record<string, unknown>;
...
export function mapRunDataToDb(data: Partial<RunData>): Partial<RunDataDbRow> {
  ...
  if (data.value !== undefined)
    dbRow.value = data.value as Record<string, unknown>;
  if (data.metadata !== undefined)
    dbRow.metadata = data.metadata as Record<string, unknown>;
