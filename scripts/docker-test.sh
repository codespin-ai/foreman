...
CONTAINER_NAME="foreman-test-$$"
TEST_DB_NAME="foreman_test_$$"
...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  IMAGE  Docker image to test (default: foreman:latest)"
    echo "  PORT   Port to expose the service on (default: 5099)"
    echo ""
    echo "Examples:"
    echo "  $0                                    # Test foreman:latest on port 5099"
    echo "  $0 ghcr.io/codespin-ai/foreman:latest # Test specific image"
    echo "  $0 foreman:latest 5002                # Test on specific port"
    exit 0
fi

# Parse command line arguments
IMAGE_TO_TEST=${1:-"foreman:latest"}
TEST_PORT=${2:-5099}

# Main test script
print_info "=== Foreman Docker Image Test ==="
echo
...
# Start the container
print_info "Starting Foreman container..."
docker run -d --rm \
    --name $CONTAINER_NAME \
    -p $TEST_PORT:5002 \
...
if ! wait_for_service localhost $TEST_PORT "Foreman REST API server"; then
